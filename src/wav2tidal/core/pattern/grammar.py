...
Grammar v3 has three start rules: ``line`` (a v2 Tidal config line),
``mini`` (the quoted mini-notation alone — what the scheduler
interprets), and ``scene`` (a parameter scene: voices + trajectories +
optional layer, design-change-002).
"""
...
def get_parser() -> Lark:
    return Lark(
        grammar_path().read_text(), parser="earley", start=["line", "mini", "scene"]
    )
...
def parse_scene(text: str) -> Tree:
    """Parse a parameter-scene config (grammar v3). Raises LarkError."""
    return get_parser().parse(text, start="scene")


def mini_text(node: Tree) -> str:
    """Reconstruct canonical mini-notation text from a parsed subtree."""
    if not isinstance(node, Tree):
        return str(node)
    if node.data in ("mini", "line"):
        return mini_text(node.children[0])
    if node.data == "sequence":
        return " ".join(mini_text(c) for c in node.children)
    if node.data == "rest":
        return "~"
    if node.data == "event":
        name = str(node.children[0])
        return f"{name}:{node.children[1]}" if len(node.children) > 1 else name
    if node.data == "modified":
        return mini_text(node.children[0]) + str(node.children[1])
    if node.data == "group":
        return f"[{mini_text(node.children[0])}]"
    if node.data == "stack":
        return ", ".join(mini_text(c) for c in node.children)
    raise ValueError(f"not a mini-notation node: {node.data}")
